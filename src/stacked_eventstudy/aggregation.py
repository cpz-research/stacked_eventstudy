...
from stacked_eventstudy.utils import make_confidence_interval, standard_error_from_variance
...
def compute_cohort_weights(stacked_data: pd.DataFrame) -> pd.DataFrame:
    ...
    treated_counts = (
        stacked_data.loc[stacked_data["treated_in_subevent"] == 1, ["subevent", "unit_id"]]
        .drop_duplicates()
...
def aggregate_cohort_params(
    ...
    vcov_rows: dict[int, dict[int, float]] = {event_time: {} for event_time in event_times}

    for event_time in event_times:
        event_params = cohort_params.loc[cohort_params["event_time"] == event_time].merge(
            cohort_weights,
            on="subevent",
            how="left",
        ...
        variance = float(aligned_weights.to_numpy() @ covariance_block.to_numpy() @ aligned_weights.to_numpy())
